...
{
    const vec2* v = reinterpret_cast<const vec2*>( luaL_checkudata(lua_state, 1, VEC2) );
...
{
    const vec2* v0 = reinterpret_cast<const vec2*>( luaL_checkudata(lua_state, 1, VEC2) );
    const vec2* v1 = reinterpret_cast<const vec2*>( luaL_checkudata(lua_state, 2, VEC2) );
...
{
    const vec2* v0 = reinterpret_cast<const vec2*>( luaL_testudata(lua_state, 1, VEC2) );
    const vec2* v1 = reinterpret_cast<const vec2*>( luaL_testudata(lua_state, 2, VEC2) );
    if ( v0 && v1 )
    {
        return vec2_push( lua_state, *v0 * *v1 );        
    }
    ...
    return luaL_error( lua_state, "invalid arguments to vec2 multiply" );
}
